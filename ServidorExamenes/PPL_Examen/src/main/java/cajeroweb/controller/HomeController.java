...
public class HomeController {
    ...
    @GetMapping("/prestamos/home")
    public String mostrarHomePrestamo(HttpSession sesion, Model model) {
        Cuenta cuenta = (Cuenta) sesion.getAttribute("cuenta");
        if (cuenta == null) {
            return "redirect:/login"; 
        }
        ...
        List<Prestamo> prestamos = pdao.buscarPrestamoPorCuenta(cuenta);
        model.addAttribute("prestamos", prestamos);
        return "HomePrestamo"; 
    }

   
    @GetMapping("/prestamos/alta")
    public String mostrarFormularioPrestamo(HttpSession sesion, RedirectAttributes ratt) {
        if (sesion.getAttribute("cuenta") == null) {
            ratt.addFlashAttribute("error", "No hay cuenta activa.");
            return "redirect:/login"; 
        }
    ...
    @PostMapping("/prestamos/alta")
    public String altaPrestamo(
        @RequestParam String descripcion,
        @RequestParam double cantidadPrestamo,
        ...
        cdao.modificarCuenta(cuenta, cantidadPrestamo, true);
        pdao.registrarPrestamo(cuenta, descripcion, cantidadPrestamo, tasaInteresAnual, plazoMeses, tipoCuota);

        ratt.addFlashAttribute("mensaje", "Préstamo registrado exitosamente y añadido al saldo.");

        return "redirect:/prestamos/home";
    }

    @GetMapping("/prestamos/eliminar/{idPrestamo}")
    public String eliminarPrestamo(@PathVariable("idPrestamo") int idPrestamo, RedirectAttributes ratt) {
        int resultado = pdao.deleteUno(idPrestamo); // Usamos el método deleteUno del DAO
        if (resultado == 1) {
            ratt.addFlashAttribute("mensaje", "Préstamo eliminado exitosamente.");
        } else {
            ratt.addFlashAttribute("error", "No se pudo eliminar el préstamo. Verifique si existe.");
        }
        return "redirect:/prestamos/home"; 
    }
